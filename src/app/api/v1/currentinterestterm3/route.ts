...
// GET /api/v1/currentinterestterm3
...
export function getPendingInterestReport(inputMonth: string = '2025-04') {
  ...
  const query = `
    ...
    InterestDateCalc AS (
        SELECT *,
            -- Calculate how many quarters have passed until the input month
            ((strftime('%Y', InputMonthStart) - strftime('%Y', StartDate)) * 12 + 
            (strftime('%m', InputMonthStart) - strftime('%m', StartDate))) / 3 AS QuarterCount
        FROM BaseData
    ),
    ExpectedInterestDate AS (
        SELECT *,
            DATE(StartDate, '+' || (CAST(QuarterCount AS INTEGER) * 3) || ' months') AS InterestDate
        FROM InterestDateCalc
    ),
    CurrentMonthPending AS (
        ...
        WHERE p.Status != 'Closed' AND p.Term = 3
    )
    ...
    SELECT
        ...
        eid.Zone,
        eid.Term,
        DATE(eid.StartDate) AS StartDate,
        DATE(eid.InterestDate) AS InterestDate,
    ...
    FROM ExpectedInterestDate eid
    JOIN CurrentMonthPending cmp ON cmp.PrincipalID = eid.PrincipalID
    WHERE cmp.CurrentMonthPendingInterest > 0
    AND strftime('%Y-%m', eid.InterestDate) = eid.InputMonthStr
    AND eid.InputMonthStr > strftime('%Y-%m', eid.StartDate) 
    ORDER BY eid.PrincipalID;
        `;
...
// POST /api/v1/currentinterestterm3
...
// PUT /api/v1/currentinterestterm3
