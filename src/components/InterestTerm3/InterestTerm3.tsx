...
const Clients = () => {
  ...
  const fetchCurrentInterest = (selectedMonth: string) => {
    ...
    fetch(`/api/v1/currentinterestterm3?month=${selectedMonth}`)
      .then((res) => res.json())
      .then((rows) => {
  ...
  const fetchPreviousPending = (selectedMonth: string) => {
    ...
    fetch(`/api/v1/term3?month=${selectedMonth}`)
      .then((res) => res.json())
      .then((rows) => {
  ...
  const handleReceived = async () => {
    ...
    if (toInsert.length > 0) {
      await fetch("/api/v1/currentinterestterm3", {
        method: "POST",
  ...
  const handleDownloadMonthlyPendingPDF = useCallback(async () => {
    ...
    const now = new Date();
    const filename = `Quarterly Pending Interests_${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, "0")}${String(now.getDate()).padStart(2, "0")}_${String(now.getHours()).padStart(2, "0")}${String(now.getMinutes()).padStart(2, "0")}${String(now.getSeconds()).padStart(2, "0")}.pdf`;
